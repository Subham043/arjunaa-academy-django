...
class Category(TimestampInfo): #extends timestamp info abstract class
    ...
    uploaded_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="course_categories_uploaded") #using the user model here for author forign key
...
class Course(TimestampInfo): #extends timestamp info abstract class
    ...
    is_draft = models.BooleanField(default=False)
    category = models.ForeignKey(Category, on_delete=models.SET_NULL, null=True, blank=True, related_name="courses") #using the user model here for author forign key
    uploaded_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="courses_uploaded") #using the user model here for author forign key
    meta_title = models.TextField(blank=True, null=True)
    og_title = models.TextField(blank=True, null=True)
    meta_description = models.TextField(blank=True, null=True)
    og_description = models.TextField(blank=True, null=True)
